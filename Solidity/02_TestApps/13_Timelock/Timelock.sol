...
contract Timelock {
    ...
    function queue(address _target, uint _value, string calldata _func, bytes calldata _data, uint _timestamp) external onlyOwner returns(bytes32 txId) {
    ...
    function execute(address _target, uint _value, string calldata _func, bytes calldata _data, uint _timestamp) external onlyOwner returns(bytes memory) {
        ...
        (bool ok, bytes memory res) = payable(_target).call{value:_value}(data);
        if(!ok) {
